function SMU_DMM_combined_control_V_bias_applied(compliance, start, stop, step, delay, testname)
    %%%This function applies a voltage bias!!! 
    ...
    % Input arguments:
    % Compliance(floating point number) = maximum value of current allowed to be measured,
    ...
    % Subplot 1 = I_meas-V_meas scatter plot
    % Subplot 2 = I_meas-V_bias scatter plot
    % Subplot 3 = log(|R|)-V_bias scatter plot
    ...
    for i=1 :length(V_app)
        ...
        fprintf(keith236, bias); %Applying ith element of applied voltage array, delay before applying it specified at top of code, autorange (0)
    ...
    %I_meas vs V_bias
    subplot(2,2,2)
    scatter(V_app, I_meas) %Log(I)-V plot
    xlabel('V_{bias}(V)')
    ylabel('I_{meas}(A)')
    
    %log(R) against bias voltage
    R = V_meas./I_meas;
    y=log10(abs(R));
    subplot(2,2,3)
    scatter(V_app, y)
